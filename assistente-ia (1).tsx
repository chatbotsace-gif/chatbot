...
export default function AssistenteIA() {
  ...
  const handleLogin = () => {
    ...
    setCurrentUser({ name: loginUsername, isAdmin: loginUsername === 'admin' });
    setIsLoggedIn(true);
    setMessages([{ 
      text: `🎉 Bem-vindo, ${loginUsername}! Como posso ajudar hoje?`, 
      isUser: false, 
      id: Date.now() 
    }]);
    setLoginUsername('');
  ...
  const handleRegister = () => {
    ...
    setUsers(prev => ({ ...prev, [regUsername]: regPassword }));
...
🔥 Pronto para usar! 🚀`;
    ...
    // Retorna null para indicar que deve usar a IA
    return null;
  };

  const callGeminiAPI = async (text) => {
    try {
      const GEMINI_KEY = 'AIzaSyAyIIBj2Kf3SRrlhXbNXB_9b11pSSttHB4';
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `Responda em português brasileiro de forma amigável, clara e resumida (máximo 200 palavras): ${text}`
            }]
          }],
          generationConfig: {
            temperature: 0.7,
      ...
      return processLocalResponse(text);
    }
  };
  ...
  const sendMessage = async () => {
    ...
    // Tenta resposta local primeiro (cálculos, hora, piadas, etc)
    const localResponse = processLocalResponse(userMessage);
    
    // Se tiver resposta local, usa ela
    if (localResponse) {
      setTimeout(() => {
      ...
      // Caso contrário, chama a API do Gemini
  ...
  return (
    <div className="flex flex-col h-screen bg-gradient-to-br from-slate-900 to-slate-800 text-slate-100">
      ...
      {showAdmin && currentUser?.isAdmin && (
        <div className="fixed top-0 right-0 w-96 h-full bg-slate-800 border-l-2 border-blue-500 shadow-2xl p-6 overflow-y-auto animate-slideIn z-50">
          <div className="flex justify-between items-center mb-6 pb-4 border-b-2 border-blue-500">
            ...
            <button
              ...
              className="w-8 h-8 bg-red-500 rounded-full hover:bg-red-600 transition-all flex items-center justify-center"
            >
              ✕
            </button>
          </div>
